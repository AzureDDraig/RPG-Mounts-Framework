...
public class JavaModelLoader {
            ...
            if (models != null) {
                models.keySet().removeIf(key -> {
                    if (key instanceof ResourceLocation loc) {
                        return loc.getNamespace().equals("rpg_mounts");
                    }
                    return key.toString().startsWith("rpg_mounts:");
                });
            }
    ...
    public static void loadJavaModel(String modelId) {
        ...
        if (!unpackedFolder.exists() || !unpackedFolder.isDirectory()) {
            RPGMounts.LOGGER.warn("[RPG_MOUNTS] Unpacked model folder does not exist: " + unpackedFolder.getAbsolutePath() + ". If this mount is not using a custom model, please set its Model Type to 'Vanilla' in the Mount Creator UI.");
    ...
    public static void loadTexture(String mountId, String modelId, String texturePath) {
        ...
        // If the texturePath is a direct resource location (e.g. minecraft:textures/...) or has a colon, load it directly
        if (texturePath != null && texturePath.contains(":")) {
            try {
                ...
                RPGMounts.LOGGER.info("[RPG_MOUNTS_DEBUG_NOTIF] Resolved texturePath as direct ResourceLocation: " + loc);
                return;
            } catch (Exception e) {
                RPGMounts.LOGGER.warn("[RPG_MOUNTS_DEBUG_NOTIF] Failed to parse texturePath as ResourceLocation: " + texturePath, e);
            }
        }
        ...
        if (!unpackedFolder.exists() || !unpackedFolder.isDirectory()) {
            RPGMounts.LOGGER.warn("[RPG_MOUNTS_DEBUG_NOTIF] Unpacked texture folder does not exist or is not a directory: " + unpackedFolder.getAbsolutePath() + ". Using default texture.");
            textureCache.put(mountId, RPGMountRenderer.DEFAULT_TEXTURE);
            return;
        }
        ...
        if (pngFile != null && pngFile.exists()) {
            try (FileInputStream fis = new FileInputStream(pngFile)) {
                ...
                RPGMounts.LOGGER.warn("[RPG_MOUNTS_DEBUG_NOTIF] Exception occurred while loading texture for mount: " + mountId + ". Using default texture.", e);
                textureCache.put(mountId, RPGMountRenderer.DEFAULT_TEXTURE);
            }
        } else {
            RPGMounts.LOGGER.warn("[RPG_MOUNTS_DEBUG_NOTIF] No valid texture (.png) file found for mount: " + mountId + " in folder: " + unpackedFolder.getAbsolutePath() + ". Using default texture.");
            textureCache.put(mountId, RPGMountRenderer.DEFAULT_TEXTURE);
        }
    }
